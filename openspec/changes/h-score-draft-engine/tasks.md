...
- [ ] 3.16 **H₀ models the wrong room — wire the field's board into the opponent model.** Follows from 3.15. `HScoreEngine` prices the opposing side's unknown picks with `_weighted_future(avail, neutral)`, a softmax over *our own* board, so its model of the room is "everyone else drafts best-available on the G-score board" — literally true in the published setup, wrong anywhere else. Corroborating from the other direction: `adp_ranks` and `survival_probability`, task 3.5's scarcity model, have **no callers** in `fantasy_gm/` or `scripts/` — defined, tested in isolation, unreachable, the same shape as `assignment.py` (3.14). `PickScore.survival` is documented as "if an ADP model was given"; nothing has ever given one.
      ...
      **Measured 2026-08-26, and the hypothesis is refuted.** 2025-26, both objectives, both arms, with and without idle weeks, ADP field throughout, 2000 seasons x 12 seats against each cell's own null. Telling H₀ the field's board made it **worse in 7 of 8 cells** (mean -0.61pp category rate; the eighth is +0.12pp, inside noise) and seat counts never improved. The deficit is not a wrong belief about what the room will draft. Consistent with a sharper reading of 3.15: H₀'s edge in the paper's room is less "it models the room" than "the room shares its valuation, so the pool it plans around is the pool that survives". **3.14 (positional assignment) is now the leading remaining explanation.** The survival half stays unwired — a design decision, not a connection. See `results.md` and `runs/opponent-board/`
