...
- [x] 2.10 **Backfill 2024-25** — required for the backtest in 2.11. **Done 2026-08-17**, run by the user from their own network. The store now holds three complete seasons: 2023-24 (26,401 logs / 572 players), 2024-25 (26,306 / 569), 2025-26 (26,651 / 582). This unblocks 2.11's cross-season gate *and* retires the single-season caveat on the draft replay
...
- [ ] 3.8 Validate against the published simulation results as a correctness check before trusting the real-data numbers. **Still the priority**: 3.12 explains part of the deficit but H₀ still loses to the static G-score board, so the question remains whether this implementation reproduces the paper at all
- [ ] 3.9 Local-optimum audit: multi-start on a subset of picks, measure the objective gap, raise multi-start only where it matters (A-DRAFT-9). **More Adam steps are ruled out** (5→20 moved the result 0.3pp); multi-start is a different remedy and is untried. **Promoted by 3.12** — with the opponent model partly exonerated, local optima and the future-pick softmax are the leading remaining causes
- [x] 3.12 **Test the representative-opponent simplification** — leading suspect for H₀'s underperformance. H₀ optimizes against one opponent but is graded against eleven, so a conceded category is far more expensive than its objective believes. **Measured: real but partial.** `OpponentModel.FIELD` closes 43% of the matchup gap and 24% of the category gap vs the static board; H₀ still loses. The informative half is that `STRONGEST` — a *tougher* single stand-in — is the worst arm of all, so the defect is the single-opponent reduction itself, not the stand-in's calibre. Also found: the shipped selector picks by roster *length*, which a snake draft holds near-constant, so "the strongest opposing team" was really a fixed seat. See `results.md`
- [ ] 3.13 **Schedule-based grading as a robustness check** — all-play-all removes schedule luck but may systematically penalise the concentrated builds H₀ discovers. Cuts both ways; needs measuring, not arguing. **Harness landed** (`round_robin_pairings`, `score_rosters(schedule=True)`); the three-season run is in flight
