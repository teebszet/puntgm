...
- [ ] 3.14 **Wire positional assignment into the objective.** `draft/assignment.py` has no callers: task 3.3 built the Jonker-Volgenant solver, pinned it against brute force, and nothing ever consumed it. Positional assignment is a named component of the published H₀ — it is how the optimizer keeps flex slots open for the picks it intends to make, and its output feeds the future-pick term directly. 3.8's reproduction matches the published number *without* it, in a room that constrains neither side; a live draft constrains both. Largest known gap between our H₀ and the paper's
      **Step 0 done 2026-08-26 — and it found two blockers that make this a three-sided change, not a one-module one.**
      (a) **`assignment.py` could not have placed anybody against our own store.** `RosterSlot` speaks PG/SG/SF/PF/C; `player_positions` speaks G/F/C/G-F/... and `PlayerPosition.slots()` passes those through unchanged, so `accepts()` intersected two disjoint vocabularies. Every guard and every forward was ineligible for every slot; centres placed only because `C` is spelled the same in both. `eligible_positions` / `slot_eligibility` in `settings.py` expand the coarse listing, pinned by a regression test. The expansion is deliberately **looser** than Yahoo's real eligibility (any listed guard fills either guard slot), which biases any measured effect **towards zero** — a null here is bounded below by the input, not evidence about the method
      (b) **Nothing in this repo starts a lineup.** `papersim._roster_week` and `replay._team_week` both sum all thirteen players every week. Wiring assignment into the objective alone would optimise a quantity neither grader measures, guaranteeing a loss that is an artifact rather than a finding. The grading side has to land first, flag-gated, since it moves every number in `results.md`. Lineups are set from pre-season expected value, never realized weeks — assigning on realized production would be an oracle
      **Scarcity audit (`scripts/position_coverage.py`), and it scopes the whole experiment:** centre is the only binding position — 31/30/31 centre-eligible in a 156-man pool against 24 forced by the slot structure (**1.25–1.29×**). Guards and forwards run ~6× over-supplied, so those slots never bind. **Every effect positional assignment can have in our data runs through centre scarcity.** Coverage is 147/148/151 of 156; the 5–9 unlisted reach the caller as `unlisted` rather than being defaulted either way (the `derive_adp_order` lesson from 3.16)
...
- [x] 3.15 **Decompose the paper room against the replay room, one axis at a time.** Two of our own experiments disagreed about the same engine: `papersim` finds H₀ ahead, `paper_engine_replay` finds it behind 12/12. Four things differed at once — the field, availability, the metric, the room design. Building it showed **the metric and the room design were already free**: the paper room reports category and matchup win rate alongside the title rate, and its single-seat swap has no adjacency artifact. `run_paper_sim` gains `field="adp"` and `include_idle_weeks=True`, both defaulting to the published setting; `scripts/room_decomposition.py` crosses them into four cells, each differenced against its own null under common random numbers.
      **Result: the FIELD owns the sign flip; availability does not.** 2025-26, `most_categories`, 2000 seasons × 12 seats: `h_paper` is +3.64pp category rate in a G-score field and +3.75pp with idle weeks added — availability moves it by nothing — but **−4.12pp** the moment the field becomes ADP bots. Seat counts are the cleanest read: **10–12 of 12 seats ahead in a G-score field, 0–1 of 12 in an ADP field.** Note this cuts against the prior earned elsewhere: the *board's* edge over z-score is entirely availability, and **H₀'s deficit is not** — different comparison, and the result does not transfer.
      Also measured: the paper correction cuts both ways. `h_paper` beats `h_score` in a G-score field (+1.19 → +3.64pp) and loses to it in an ADP field (−3.81 → −4.12pp). `future_slices` sharpens H₀'s commitment to its own model of the room, which pays when the model is right and costs when it is wrong. Regression check: at default flags the `paper` cell reproduces 3.8's recorded numbers to the digit. See `results.md` and `runs/room-decomposition/`
      **Generalized 2026-08-26 to three seasons x both objectives -- 48 arm-cells, and the sign is unanimous 48/48.** Every G-score-field cell is positive on all three metrics in all six runs; every ADP-field cell is negative in all six. Seat counts at scale: **253/288 seat-cells ahead in a G-score field, 39/288 in an ADP field.** Idle weeks still move nothing (+0.20pp mean for `h_score`, -0.56pp for `h_paper`, both inside the run-to-run range). One sub-claim narrows: `h_paper` beating `h_score` holds 6/6 in the paper's room (+2.31pp mean) but the ADP-field version was a one-run artifact -- the two arms swap places 3/6 there
- [ ] 3.16 **H₀ models the wrong room — wire the field's board into the opponent model.** Follows from 3.15. `HScoreEngine` prices the opposing side's unknown picks with `_weighted_future(avail, neutral)`, a softmax over *our own* board, so its model of the room is "everyone else drafts best-available on the G-score board" — literally true in the published setup, wrong anywhere else. Corroborating from the other direction: `adp_ranks` and `survival_probability`, task 3.5's scarcity model, have **no callers** in `fantasy_gm/` or `scripts/` — defined, tested in isolation, unreachable, the same shape as `assignment.py` (3.14). `PickScore.survival` is documented as "if an ADP model was given"; nothing has ever given one.
      **Opponent-future term built** (`opponent_board`, `model_the_field`): the ordering is substituted into the board's own score ladder rather than rescored, so opponents value a different player at each rank while the spread across ranks is unchanged and `softmax_temp` keeps meaning what it meant. Pinned by an identity test — passing the G-score board reproduces the default engine's candidate values exactly. **The survival half is still unwired**; how survival should enter the objective is a design decision, not a connection.
      **A defect in this change, caught before it became a finding:** `derive_adp_order` returns the whole league (582 players against a 156-man scored pool), so zipping the raw ordering against the ladder handed a flat 0.0 to every pool player past rank 156 — telling the engine its opponents pick near-uniformly from most of the board. Worth 2.1pp of category rate on a smoke, *larger than the effect under test*. The identity test missed it because it passes the pool itself, where the filter is a no-op
